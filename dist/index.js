let users = [];
async function getPublicRepositories(name) {
    try {
        ...
        console.log(`Usuário ${newUser.name} apelido ${newUser.login} biografia ${newUser.bio} login ${newUser.login} repositorios publicos ${newUser.public_repos} quantidade de repositorios ${newUser.repos_url}`);
        users.push(newUser);
        return newUser;
    }
    catch (err) {
...
function calculateRepositories() {
    const repositorios = users.reduce((accum, user) => accum + user.public_repos, 0);
    console.log(repositorios);
}
setTimeout(calculateRepositories, 1000);
async function maiorRepositorio() {
    const repositorios = users.reduce((maiorRepositorio, user) => user.public_repos > maiorRepositorio.public_repos ? user : maiorRepositorio);
    ...
    console.log(`O maior Repositorio é de ${repositorios.name} ele possui ${repositorios.public_repos} repositorios UAU =) 
